...
export default {
  async fetch(request, env) {
    ...
    // Rutas para SERIES DE URL
    if (request.method === 'GET' && pathname === '/series-url') {
      try {
    ...
    // Ruta para obtener solo los nombres de las series
    if (request.method === 'GET' && pathname === '/nombres-series-url') {
      try {
        const result = await env.animes_plus.prepare("SELECT DISTINCT nombreSerie FROM animes_series_enlaces").all();
        const nombresSeries = result.results.map(r => r.nombreSerie);
    ...
    // Obtener temporadas de una serie
    if (request.method === 'GET' && pathname === '/temporadas-url') {
      try {
        const nombreSerie = url.searchParams.get('serie');
        if (!nombreSerie) {
          return new Response(JSON.stringify({ error: "Falta el parámetro 'serie'" }), { status: 400, headers: corsHeaders });
        ...
        const result = await env.animes_plus
          .prepare("SELECT DISTINCT temporada FROM animes_series_enlaces WHERE nombreSerie = ?")
          .bind(nombreSerie)
          .all();
    ...
    // Obtener idiomas de una serie + temporada
    if (request.method === 'GET' && pathname === '/idiomas-url') {
      try {
        const nombreSerie = url.searchParams.get('serie');
        const temporada = url.searchParams.get('temporada');
        if (!nombreSerie || !temporada) {
        ...
        const result = await env.animes_plus
          .prepare("SELECT DISTINCT idioma FROM animes_series_enlaces WHERE nombreSerie = ? AND temporada = ?")
          .bind(nombreSerie, temporada)
          .all();
    ...
    // Obtener servidores de una serie + temporada + idioma
    if (request.method === 'GET' && pathname === '/servidores-url') {
      try {
        const nombreSerie = url.searchParams.get('serie');
        const temporada = url.searchParams.get('temporada');
        const idioma = url.searchParams.get('idioma');
        ...
        const result = await env.animes_plus
          .prepare("SELECT DISTINCT servidor FROM animes_series_enlaces WHERE nombreSerie = ? AND temporada = ? AND idioma = ?")
          .bind(nombreSerie, temporada, idioma)
          .all();
    ...
    // Obtener episodios de una serie + temporada + idioma + servidor
if (request.method === 'GET' && pathname === '/episodios-url') {
  try {
    const nombreSerie = url.searchParams.get('serie');
    const temporada = url.searchParams.get('temporada');
    const idioma = url.searchParams.get('idioma');
    ...
    const result = await env.animes_plus
      .prepare("SELECT episodio, URL FROM animes_series_enlaces WHERE nombreSerie = ? AND temporada = ? AND idioma = ? AND servidor = ?")
      .bind(nombreSerie, temporada, idioma, servidor)
      .all();
    ...
    // Actualizar episodio - RUTA CORREGIDA
if (request.method === 'PUT' && pathname === '/actualizar-episodio-url') {
  try {
    const { serie, temporada, idioma, servidor, episodio, url } = await request.json();

    if (!serie || !temporada || !idioma || !servidor || !episodio || !url) {
      return new Response(JSON.stringify({ error: "Faltan parámetros requeridos" }), { 
        status: 400, 
        headers: corsHeaders 
      });
    ...
    const result = await env.animes_plus
      .prepare(`
        UPDATE animes_series_enlaces
        SET URL = ?
        WHERE nombreSerie = ? AND temporada = ? AND idioma = ? AND servidor = ? AND episodio = ?
      `)
      .bind(url, serie, temporada, idioma, servidor, episodio) // Cambiado de iframe a url
      .run();
    ...
    // Eliminar episodio - RUTA SIMPLIFICADA
    if (request.method === 'DELETE' && pathname === '/eliminar-episodio-url') {
      try {
        const { serie, temporada, idioma, servidor, episodio } = await request.json();
...
if (request.method === 'POST' && pathname === '/registrar-url') {
  try {
    ...
    const stmt = env.animes_plus.prepare(`
      INSERT INTO animes_series_enlaces 
      (nombreSerie, temporada, idioma, servidor, episodio, URL)
      VALUES (?, ?, ?, ?, ?, ?)
      ON CONFLICT (nombreSerie, temporada, idioma, servidor, episodio)
      DO UPDATE SET 
        URL = excluded.URL,
    ...
    for (const reg of registros) {
      await stmt
        .bind(
          ...
          reg.servidor,
          reg.episodio,
          reg.url  // Cambiado de iframe a url
        )
        .run();
    }
 ...
 // Rutas para SERIES DE DESCARGAS
    if (request.method === 'GET' && pathname === '/series-descargas') {
      try {
    ...
    // Ruta para obtener solo los nombres de las series
    if (request.method === 'GET' && pathname === '/nombres-series-descargas') {
      try {
        const result = await env.animes_plus.prepare("SELECT DISTINCT nombreSerie FROM animes_series_descargas").all();
        const nombresSeries = result.results.map(r => r.nombreSerie);
    ...
    // Obtener temporadas de una serie
    if (request.method === 'GET' && pathname === '/temporadas-descargas') {
      try {
        const nombreSerie = url.searchParams.get('serie');
        if (!nombreSerie) {
          return new Response(JSON.stringify({ error: "Falta el parámetro 'serie'" }), { status: 400, headers: corsHeaders });
        ...
        const result = await env.animes_plus
          .prepare("SELECT DISTINCT temporada FROM animes_series_descargas WHERE nombreSerie = ?")
          .bind(nombreSerie)
          .all();
    ...
    // Obtener idiomas de una serie + temporada
    if (request.method === 'GET' && pathname === '/idiomas-descargas') {
      try {
        const nombreSerie = url.searchParams.get('serie');
        const temporada = url.searchParams.get('temporada');
        if (!nombreSerie || !temporada) {
        ...
        const result = await env.animes_plus
          .prepare("SELECT DISTINCT idioma FROM animes_series_descargas WHERE nombreSerie = ? AND temporada = ?")
          .bind(nombreSerie, temporada)
          .all();
    ...
    // Obtener servidores de una serie + temporada + idioma
    if (request.method === 'GET' && pathname === '/servidores-descargas') {
      try {
        const nombreSerie = url.searchParams.get('serie');
        const temporada = url.searchParams.get('temporada');
        const idioma = url.searchParams.get('idioma');
        ...
        const result = await env.animes_plus
          .prepare("SELECT DISTINCT servidor FROM animes_series_descargas WHERE nombreSerie = ? AND temporada = ? AND idioma = ?")
          .bind(nombreSerie, temporada, idioma)
          .all();
    ...
    // Obtener episodios de una serie + temporada + idioma + servidor
if (request.method === 'GET' && pathname === '/episodios-descargas') {
  try {
    const nombreSerie = url.searchParams.get('serie');
    const temporada = url.searchParams.get('temporada');
    const idioma = url.searchParams.get('idioma');
    ...
    const result = await env.animes_plus
      .prepare("SELECT episodio, URL FROM animes_series_descargas WHERE nombreSerie = ? AND temporada = ? AND idioma = ? AND servidor = ?")
      .bind(nombreSerie, temporada, idioma, servidor)
      .all();
    ...
    // Actualizar episodio - RUTA CORREGIDA
if (request.method === 'PUT' && pathname === '/actualizar-episodio-descargas') {
  try {
    const { serie, temporada, idioma, servidor, episodio, url } = await request.json();
    ...
    const result = await env.animes_plus
      .prepare(`
        UPDATE animes_series_descargas
        SET URL = ?
        WHERE nombreSerie = ? AND temporada = ? AND idioma = ? AND servidor = ? AND episodio = ?
      `)
      .bind(url, serie, temporada, idioma, servidor, episodio) // Cambiado de iframe a url
    ...
    // Eliminar episodio - RUTA SIMPLIFICADA
    if (request.method === 'DELETE' && pathname === '/eliminar-episodio-descargas') {
      try {
        const { serie, temporada, idioma, servidor, episodio } = await request.json();
...
if (request.method === 'POST' && pathname === '/registrar-descargas') {
  try {
    ...
    const stmt = env.animes_plus.prepare(`
      INSERT INTO animes_series_descargas 
      (nombreSerie, temporada, idioma, servidor, episodio, URL)
      VALUES (?, ?, ?, ?, ?, ?)
      ON CONFLICT (nombreSerie, temporada, idioma, servidor, episodio)
      DO UPDATE SET 
 ...
 // Rutas para SERIES ORIGINAL
    if (request.method === 'GET' && pathname === '/series-original') {
      try {
    ...
    // Ruta para obtener solo los nombres de las series
    if (request.method === 'GET' && pathname === '/nombres-series-original') {
      try {
        const result = await env.animes_plus.prepare("SELECT DISTINCT nombreSerie FROM animes_series_original").all();
        const nombresSeries = result.results.map(r => r.nombreSerie);
    ...
    // Obtener temporadas de una serie
    if (request.method === 'GET' && pathname === '/temporadas-original') {
      try {
        const nombreSerie = url.searchParams.get('serie');
        if (!nombreSerie) {
          return new Response(JSON.stringify({ error: "Falta el parámetro 'serie'" }), { status: 400, headers: corsHeaders });
        ...
        const result = await env.animes_plus
          .prepare("SELECT DISTINCT temporada FROM animes_series_original WHERE nombreSerie = ?")
          .bind(nombreSerie)
          .all();
    ...
    // Obtener idiomas de una serie + temporada
    if (request.method === 'GET' && pathname === '/idiomas-original') {
      try {
        const nombreSerie = url.searchParams.get('serie');
        const temporada = url.searchParams.get('temporada');
        if (!nombreSerie || !temporada) {
        ...
        const result = await env.animes_plus
          .prepare("SELECT DISTINCT idioma FROM animes_series_original WHERE nombreSerie = ? AND temporada = ?")
          .bind(nombreSerie, temporada)
          .all();
    ...
    // Obtener servidores de una serie + temporada + idioma
    if (request.method === 'GET' && pathname === '/servidores-original') {
      try {
        const nombreSerie = url.searchParams.get('serie');
        const temporada = url.searchParams.get('temporada');
        const idioma = url.searchParams.get('idioma');
        ...
        const result = await env.animes_plus
          .prepare("SELECT DISTINCT servidor FROM animes_series_original WHERE nombreSerie = ? AND temporada = ? AND idioma = ?")
          .bind(nombreSerie, temporada, idioma)
          .all();
    ...
    // Obtener episodios de una serie + temporada + idioma + servidor
if (request.method === 'GET' && pathname === '/episodios-original') {
  try {
    const nombreSerie = url.searchParams.get('serie');
    const temporada = url.searchParams.get('temporada');
    const idioma = url.searchParams.get('idioma');
    ...
    const result = await env.animes_plus
      .prepare("SELECT episodio, URL FROM animes_series_original WHERE nombreSerie = ? AND temporada = ? AND idioma = ? AND servidor = ?")
      .bind(nombreSerie, temporada, idioma, servidor)
      .all();
    ...
    // Actualizar episodio - RUTA CORREGIDA
if (request.method === 'PUT' && pathname === '/actualizar-episodio-original') {
  try {
    const { serie, temporada, idioma, servidor, episodio, url } = await request.json();
    ...
    // Eliminar episodio - RUTA SIMPLIFICADA
    if (request.method === 'DELETE' && pathname === '/eliminar-episodio-original') {
      try {
        const { serie, temporada, idioma, servidor, episodio } = await request.json();
...
if (request.method === 'POST' && pathname === '/registrar-original') {
  try {
    ...
    const stmt = env.animes_plus.prepare(`
      INSERT INTO animes_series_original 
      (nombreSerie, temporada, idioma, servidor, episodio, URL)
      VALUES (?, ?, ?, ?, ?, ?)
      ON CONFLICT (nombreSerie, temporada, idioma, servidor, episodio)
      DO UPDATE SET 
