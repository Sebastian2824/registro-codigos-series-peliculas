      ...
      datosPagina.forEach(d => {
        html += `
          <tr>
            ...
            <td class="iframe-cell">${truncateText(escapeHTML(d.iframe), 50)}</td>
          </tr>
        `;
      });
              ...
              for (const servidorDoc of servidoresSnapshot.docs) {
                ...
                for (const episodioDoc of episodiosSnapshot.docs) {
                  datosFirebase.push({
                    ...
                    iframe: episodioDoc.data().iframe || ''
                  });
                }
              }
            }
    ...
    btnTransferir.addEventListener('click', async function() {
      ...
      try {
        for (let i = 0; i < total; i += chunkSize) {
          ...
          let response;
          try {
            response = await fetch("https://proyect-cloud-flare.apiprueba2025.workers.dev/registrar-descargas", {
              method: "POST",
