      ...
      datosPagina.forEach(d => {
        html += `
          <tr>
            ...
            <td class="iframe-cell">${escapeHTML(truncateText(d.iframe, 50))}</td>
          </tr>
        `;
      });
    ...
    btnCargar.addEventListener('click', async function() {
      try {
        ...
        for (const serieDoc of seriesSnapshot.docs) {
          ...
          loadingProgressBar.textContent = `${progressPercent}%`;
    ...
    btnTransferir.addEventListener('click', async function() {
      ...
      try {
        for (let i = 0; i < total; i += chunkSize) {
          ...
          let response;
          try {
            response = await fetch("https://proyect-cloud-flare.apiprueba2025.workers.dev/registrar", {
              method: "POST",
